...
- **Testnet only.** 3 single-operator verifiers; mocked off-chain evidence; WCSPR/test-CSPR.

**Pending (RECTOR's gate):** the bundled testnet deploy (new contract hash + e2e smoke incl. cross-side TS-sign→Rust-verify byte-consistency + the SPEC-3 tail: README hashes + BUIDL sample txs), then merge to `main`.

---

## Next — post-hackathon (~Q1 2026)

- **One real payment-rail adapter** — replace one mocked verifier with a real data source (Stripe test → production; or a bank API). The quorum is untouched; only the evidence source upgrades. Closes the "mocked evidence" disclosure.
